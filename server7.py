...
sp = spotipy.Spotify(auth_manager=SpotifyOAuth(
    ...
    scope="user-modify-playback-state user-read-playback-state user-library-modify playlist-modify-private user-read-recently-played"
))
...
def current_song():
    ...
    try:
        ...
        return jsonify({
            ...
            "is_liked": track["uri"] in liked_songs
        }), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def like_song():
    ...
    try:
        ...
        if song_uri not in liked_songs:
            liked_songs.append(song_uri)

        return jsonify({"message": "Song added to liked songs", "is_liked": True}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route('/dislike-song', methods=['POST'])
def dislike_song():
    """Remove a song from liked songs and any created playlist"""
    try:
        data = request.get_json()
        song_uri = data.get('uri')
        ...
        # Remove from local liked songs list
        if song_uri in liked_songs:
            liked_songs.remove(song_uri)

        return jsonify({"message": "Song removed from liked songs", "is_liked": False}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route('/create-playlist', methods=['POST'])
def create_playlist():
    """Create a playlist from all liked songs"""
    try:
        if not liked_songs:
            return jsonify({"error": "No liked songs to add to playlist"}), 400

        data = request.get_json()
        playlist_name = data.get('name', f"My Liked Songs {datetime.now().strftime('%m-%d')}")
        ...
        sp.playlist_add_items(playlist['id'], liked_songs)

        return jsonify({
            "message": "Playlist created successfully",
            "playlist": {
                "name": playlist['name'],
                "track_count": len(liked_songs)
            }
        }), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
